$(function() {
  ...
  socket.on("onlineStack", function(stack) {
    ...
    $("#list").append(
      $("<li>").append(
        $('<button id="ubtn" class="btn btn-info btn-block"></button>')
          .text("Click here to chat in Group")
          .css({ "font-size": "12px" })
      )
    );
    var totalOnline = 0;
    for (var user in stack) {
      ...
      if (stack[user] == "Online") {
        var txt2 = $('<span class="badge"></span>')
          .text(stack[user])
          .css({ float: "right", color: "#009933", "font-size": "14px" });
        totalOnline++;
      } else {
        var txt2 = $('<span class="badge"></span>')
          .text(stack[user])
          .css({ float: "right", color: "#a6a6a6", "font-size": "14px" });
      }
      //listing all users.
    ...
    if (data.room == roomId) {
      ...
      if (data.result.length != 0) {
        ...
        for (var i = 0; i < data.result.length; i++) {
          ...
          var txt2 = $("<span></span>")
            .text(chatDate)
            .css({ float: "right", color: "#a6a6a6", "font-size": "10px" });
          var txt3 = $("<p></p>").append(txt1, txt2);
          var txt4 = $("<p></p>")
            .text(data.result[i].msg)
            .css({ color: "#000000", "font-size": "12px" });
  ...
  socket.on("chat-msg", function(data) {
    ...
    var txt2 = $("<span></span>")
      .text(chatDate)
      .css({ float: "right", color: "#a6a6a6", "font-size": "12px" });
    var txt3 = $("<p></p>").append(txt1, txt2);
    var txt4 = $("<p></p>")
      .text(data.msg)
      .css({ color: "#000000", "font-size": "12px" });
